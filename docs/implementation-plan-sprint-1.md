...
Sprint 1 covers FR-001 to FR-008, FR-074, FR-075, FR-107 to FR-113, FR-129, FR-130. The goal is to deliver the full foundation: 5 layout shells (DefaultLayout, HomeLayout, AuthLayout, AppLayout, StudioLayout), multi-language i18n, auth suite, Series CRUD, and pluggable DB/AI infrastructure.
...
Series/Project Hub dashboard using `AppLayout`:
...
1. Landing page renders HomeLayout (no sidebar)
2. Auth flow: Navigate to `/auth/login`, fill form, login → redirect to `/dashboard`
3. Dashboard: Assert `AppLayout` (`.g-sub-sidebar`, `.g-main-area`)
...
- Verify 5 layout shells render correctly at `/`, `/manual`, `/auth/login`, `/dashboard`, `/editor`
