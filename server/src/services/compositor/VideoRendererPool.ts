...
export class VideoRendererPool {
  ...
  constructor(options: RendererPoolOptions = {}) {
    ...
    Logger.info(
      `[VideoRendererPool] Initialized Local Render Pool: [Min: ${this.minInstances}, Max: ${this.maxInstances}] (Detected: ${cpuCount} CPUs, ${totalMemMB}MB RAM, Safe Max: ${calculatedDefaultMax})`
    );
  ...
  private async warmup(): Promise<void> {
    for (let i = 0; i < this.minInstances; i++) {
      this.createInstance().catch((err) => {
        Logger.warn(`[VideoRendererPool] Warmup instance creation warning: ${err.message}`);
      });
    }
  }
  ...
  private async createInstance(): Promise<PooledRendererInstance> {
    ...
    try {
      Logger.info(`[VideoRendererPool] Initializing new VideoRenderer instance ${id}...`);
      await pooled.renderer.init();
      pooled.isInitializing = false;
      Logger.info(`[VideoRendererPool] VideoRenderer instance ${id} is ready for rendering.`);
      return pooled;
    } catch (err: any) {
      pooled.isInitializing = false;
      this.removeInstance(id);
      Logger.error(`[VideoRendererPool] Failed to initialize instance ${id}: ${err.message}`);
      throw err;
    }
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    if (!available && this.instances.length < this.maxInstances) {
      try {
        ...
        Logger.warn(`[VideoRendererPool] Failed to scale up instance, waiting for existing: ${err.message}`);
      }
    }
    ...
    if (waitTimeMs > 100) {
      Logger.info(`[VideoRendererPool] Job ${nextJob.id} dequeued after waiting ${waitTimeMs}ms. Assigned to instance ${available.id}.`);
    }
    ...
    try {
      ...
      if (available.rendersCount >= this.maxRendersPerInstance) {
        Logger.info(`[VideoRendererPool] Instance ${available.id} reached ${available.rendersCount} renders. Recycling instance for memory hygiene...`);
      ...
      Logger.error(`[VideoRendererPool] Render failed on instance ${available.id}: ${renderErr?.message || renderErr}`);
  ...
  private async recycleInstance(inst: PooledRendererInstance): Promise<void> {
    ...
    if (this.jobQueue.length > 0 || this.instances.length < this.minInstances) {
      this.createInstance()
        ...
        .catch((err: any) => {
          Logger.warn(`[VideoRendererPool] Instance replacement warning: ${err.message}`);
        });
    } else {
      this.processQueue();
    }
  ...
  public async destroyAll(): Promise<void> {
    ...
    Logger.info(`[VideoRendererPool] Destroying all ${this.instances.length} pooled renderer instances...`);
