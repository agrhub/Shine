...
// Track height definitions matching TimelineCanvas sizesMap
...
export function getTrackHeight(type: string): number {
  if (!type) return 48;
  const key = String(type);
  const normalized = key.charAt(0).toUpperCase() + key.slice(1).toLowerCase();
  return TRACK_HEIGHTS[key] || TRACK_HEIGHTS[normalized] || 48;
}
