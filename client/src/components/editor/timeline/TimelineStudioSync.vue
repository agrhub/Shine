...
import { clipToJSON } from '@openvideo/engine-pixi';
...
const syncStudioToStore = (studio: any) => {
  ...
  const handleClipAdded = ({ clip, trackId }: any) => {
    ...
    const from = serializedClip.display?.from ?? clip.display?.from ?? 0;
    const duration = clip.duration || 5 * 1e6;
    const to = serializedClip.display?.to ?? clip.display?.to ?? (from + duration);
    ...
    timelineState.value.clips[clip.id] = {
      ...
      duration: duration,
      display: { from, to },
      trim: serializedClip.trim || { from: 0, to: duration },
      sourceDuration: clip.meta?.duration || clip.duration || duration,
    };

    const updatedTracks = timelineState.value._tracks.map((t: any) => {
  ...
  const handleTrackAdded = ({ track, index }: any) => {
    if (timelineState.value._tracks.find((t: any) => t.id === track.id)) return;
  ...
  const handleTrackRemoved = ({ trackId }: any) => {
    const updatedTracks = timelineState.value._tracks.filter((t: any) => t.id !== trackId);
    setTracks(updatedTracks);
  };
  ...
  const handleClipUpdated = ({ clip }: any) => {
    ...
    const serializedClip = clipToJSON(clip);
    const from = serializedClip.display?.from ?? clip.display?.from ?? existing.display?.from ?? 0;
    const duration = clip.duration || existing.duration || 5 * 1e6;
    const to = serializedClip.display?.to ?? clip.display?.to ?? (from + duration);
    ...
    timelineState.value.clips[clip.id] = {
      ...
      duration: duration,
      display: { from, to },
      trim: serializedClip.trim ? { ...serializedClip.trim } : existing.trim,
    };
  };
  ...
  const handleStudioRestored = ({ clips, tracks }: any) => {
    ...
    clips.forEach((c: any) => {
      ...
      const from = serialized.display?.from ?? c.display?.from ?? 0;
      const duration = c.duration || serialized.duration || 5 * 1e6;
      const to = serialized.display?.to ?? c.display?.to ?? (from + duration);
      ...
      newClipsMap[c.id] = {
        ...
        duration: duration,
        display: { from, to },
        sourceDuration: c.meta?.duration || c.duration || duration,
      };
    });
  ...
  // Selection cross-sync (Studio <-> Core) 1:1 with React
  let prevSelectedIdsStr = '';
  const unsubCore = core.store.subscribe((state: any) => {
    const selectedIds = state.selectedIds || (state as any).selectedClipIds || [];
    const idsStr = JSON.stringify(selectedIds);
    if (idsStr !== prevSelectedIdsStr) {
      prevSelectedIdsStr = idsStr;
      const { setSelectedClips } = useStudioStore();
      if (selectedIds.length > 0 && studio) {
        const engineClips = selectedIds
          .map((id: string) => studio.timeline?.getClipById?.(id) || (core.clip as any).get?.(id) || state.clips?.[id])
          .filter(Boolean);
        if (engineClips.length > 0) {
          setSelectedClips(engineClips);
        }
      } else {
        setSelectedClips([]);
      }
...
const syncTimelineCanvasToStoreAndStudio = (canvas: any) => {
  ...
  const handleClipMovedToTrack = ({ clipId, trackId }: any) => {
    ...
    const updatedTracks = timelineState.value._tracks.map((track: any) => ({
      ...track,
      clipIds: track.clipIds.filter((id:string) => id !== clipId),
    }));

    // Add clip to target track
    const finalTracks = updatedTracks.map((track: any) => {
    ...
    const filteredTracks = finalTracks.filter((t: any) => t.clipIds.length > 0);
  ...
  const handleClipMovedToNewTrack = ({ clipId, targetIndex }: any) => {
    ...
    const filteredTracks = timelineState.value._tracks
      ...
      .filter((t: any) => t.clipIds.length > 0);
