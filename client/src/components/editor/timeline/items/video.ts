...
class Video extends Trimmable {
  ...
  constructor(props: VideoProps) {
    ...
    this.src = toAbsoluteUrl(props.src);
    this.strokeWidth = 0;

    this.transparentCorners = false;
    this.hasBorders = false;

    this.previewUrl = toAbsoluteUrl(props.preview || props.metadata?.previewUrl || props.src);
  ...
  private async loadFallbackThumbnail() {
    ...
    const isVideoUrl =
      fallbackThumbnail.includes('.mp4') ||
      fallbackThumbnail.includes('.webm') ||
      fallbackThumbnail.includes('/api/media/') ||
      fallbackThumbnail.includes('/api/uploads/');
      ...
      return new Promise<void>((resolve) => {
        ...
        video.onseeked = () => {
          try {
            ...
            const aspectRatio = (video.videoWidth && video.videoHeight) ? (video.videoWidth / video.videoHeight) : 1;
            ...
            console.warn('Fallback video frame capture failed:', e);
          }
          cleanUp();
          resolve();
        };
